...
var geekcashRPC = require('@geekcash/geekcash-rpc');
var $ = geekcashcore.util.preconditions;
var _  = geekcashcore.deps._;
var Transaction = geekcashcore.Transaction;
var Proposal = geekcashcore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to geekcashd in Node.js. Manages starting and
 * stopping geekcashd as a child process for application support, as well as connecting
 * to multiple geekcashd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function GeekCash(options) {
  if (!(this instanceof GeekCash)) {
    return new GeekCash(options);
  }
...
util.inherits(GeekCash, Service);

GeekCash.dependencies = [];

GeekCash.DEFAULT_MAX_TXIDS = 1000;
GeekCash.DEFAULT_MAX_HISTORY = 50;
GeekCash.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
GeekCash.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
GeekCash.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
GeekCash.DEFAULT_SPAWN_RESTART_TIME = 5000;
GeekCash.DEFAULT_SPAWN_STOP_TIME = 10000;
GeekCash.DEFAULT_TRY_ALL_INTERVAL = 1000;
GeekCash.DEFAULT_REINDEX_INTERVAL = 10000;
GeekCash.DEFAULT_START_RETRY_INTERVAL = 5000;
GeekCash.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
GeekCash.DEFAULT_TRANSACTION_CONCURRENCY = 5;
GeekCash.DEFAULT_INSTANTSEND_FEE = 10000;
GeekCash.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  uacomment: 'geekcashcore'
};

GeekCash.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || GeekCash.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || GeekCash.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || GeekCash.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || GeekCash.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || GeekCash.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || GeekCash.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || GeekCash.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || GeekCash.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || GeekCash.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || GeekCash.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

GeekCash.prototype._initCaches = function() {
...
GeekCash.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
GeekCash.prototype.getAPIMethods = function() {
  var methods = [
...
GeekCash.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'geekcashd/' + name, 'total:', this.subscriptions[name].length);
};

GeekCash.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'geekcashd/' + name, 'total:', this.subscriptions[name].length);
};

GeekCash.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (geekcashcore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'geekcashd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

GeekCash.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'geekcashd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
GeekCash.prototype.unsubscribeAddressAll = function(emitter) {
...
GeekCash.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = GeekCash.DEFAULT_CONFIG_SETTINGS;
...
GeekCash.prototype._parseGeekCashConf = function(configPath) {
  var options = {};
...
GeekCash.prototype._expandRelativeDatadir = function() {
...
GeekCash.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in geekcashd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in geekcashd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in geekcashd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './geekcash.conf');

  log.info('Using GeekCash config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseGeekCashConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseGeekCashConf(networkConfigPath));
  }
...
GeekCash.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of geekcashcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of geekcashcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of geekcashcore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in geekcash.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that geekcashd is undergoing a reindex. ' +
             ...
             'of geekcashcore-node services will start.');
    node._reindex = true;
  }
};

GeekCash.prototype._resetCaches = function() {
 ...
 * @private
 */
GeekCash.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
GeekCash.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

GeekCash.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('GeekCash Daemon Ready');
          callback();
        });
      });
...
GeekCash.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === geekcashcore.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

GeekCash.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === geekcashcore.Networks.testnet) {
    networkPath = 'testnet3/geekcash.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/geekcash.conf';
    }
  }
  return networkPath;
};

GeekCash.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === geekcashcore.Networks.testnet) {
...
GeekCash.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('geekcashd/hashblock', message.toString('hex'));
    }
  }

};

GeekCash.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
GeekCash.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('GeekCash Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
GeekCash.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
GeekCash.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('geekcashd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
GeekCash.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = geekcashcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('geekcashd/rawtransaction', message.toString('hex'));
    }

    var tx = geekcashcore.Transaction();
    tx.fromString(message);
    var txid = geekcashcore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

GeekCash.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = geekcashcore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('geekcashd/transactionlock', message.toString('hex'));
    }

  }
};

GeekCash.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || GeekCash.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

GeekCash.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
GeekCash.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
GeekCash.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('GeekCash Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || GeekCash.DEFAULT_REINDEX_INTERVAL);

};

GeekCash.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
GeekCash.prototype._stopSpawnedGeekCash = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/geekcashd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned geekcash process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean geekcash process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
GeekCash.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedGeekCash(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting geekcash process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('GeekCash process unexpectedly exited with code:', code);
        log.warn('Restarting geekcash child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('GeekCash process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn geekcashd.'));
      }
...
GeekCash.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to geekcashd.'));
    }
 ...
 */
GeekCash.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('GeekCash configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
GeekCash.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
GeekCash.prototype.syncPercentage = function(callback) {
  var self = this;
...
GeekCash.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, geekcashcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
GeekCash.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = geekcashcore.Script.fromAddress(delta.address);
    return {
...
GeekCash.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
GeekCash.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
GeekCash.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in geekcashcore is most recent to less recent
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getAddressTxids = function(addressArg, options, callback) {
...
GeekCash.prototype._getConfirmationsDetail = function(transaction) {
...
GeekCash.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
GeekCash.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
GeekCash.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A geekcash transaction id
 * @param {Function} callback
 */
GeekCash.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
GeekCash.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof geekcashcore.Address) {
...
GeekCash.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
GeekCash.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a geekcashcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
GeekCash.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = geekcashcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
GeekCash.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
GeekCash.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GeekCash.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
GeekCash.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a geekcashcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
GeekCash.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addTxlockToTx(tx,result) {
    tx.txlock = false;
    if (tx.feeSatoshis >= GeekCash.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.0001 check for txlock
      var rawTx = new geekcashcore.Transaction(result.hex);
      var hash = geekcashcore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
      if (self.node.services.geekcashd.zmqKnownTransactionLocks.get(id)) {
 ...
 * @param callback
 */
GeekCash.prototype.govObjectList = function(options, callback) {
  var self = this;
...
GeekCash.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
GeekCash.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
GeekCash.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
GeekCash.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
GeekCash.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
GeekCash.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
GeekCash.prototype.govObjectInfo = function (callback) {
  var self = this;
...
GeekCash.prototype.govCount = function (callback) {
  var self = this;
...
GeekCash.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
GeekCash.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
GeekCash.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
GeekCash.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GeekCash.prototype.getInfo = function(callback) {
  var self = this;
...
GeekCash.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
GeekCash.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('geekcashd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('geekcashd process did not exit'));
      }
