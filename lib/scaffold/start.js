...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for geekcashd is necessary for this upgrade with the "reindex=1" geekcash.conf option. \n' +
                 'There are changes necessary in both geekcash.conf and geekcash-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/geekcash/geekcash-node/blob/geekcashd/docs/upgrade.md \n');
        ...
        servicesConfig: {
          geekcashd: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/geekcashd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed geekcash-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './geekcash-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in geekcash-node services directory
    ...
    if (servicePackage.geekcashcoreNode) {
      serviceModule = service.name + '/' + servicePackage.geekcashcoreNode;
    }
 ...
 * @param {Node} node - The GeekCashcore Node instance
 */
