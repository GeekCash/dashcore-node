...
var BASE_PACKAGE = {
  description: 'A full GeekCash node build with GeekCashcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md'
  // dependencies: {
  //   '@geekcash/geekcash-lib': '^' + geekcashcore.version,
  //   '@geekcash/geekcash-node': version
  // }
};

/**
 * Will create a directory and geekcash.conf file for GeekCash.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createGeekCashDirectory(datadir, done) {
...
/**
 * Will create a base GeekCashcore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The geekcash database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/geekcash-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a GeekCashcore Node directory, configuration file,
 * geekcash configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the geekcashcore node configuration directory
 * @param {String} options.datadir - The path to the geekcash datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the geekcash-node directory and configuration
    ...
    function(next) {
      // Setup the GeekCash directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createGeekCashDirectory(absDataDir, next);
      } else {
        next();
      }
    },
