...
/**
 * Will remove a service from geekcash-node.json
...
/**
 * Will remove the Node.js service and from the geekcash-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The geekcash-node configuration directory
...
function remove(options, done) {
  ...
  var geekcashcoreConfigPath = path.resolve(configPath, 'geekcash-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(geekcashcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a geekcash-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to geekcash-node.json
        removeConfig(geekcashcoreConfigPath, service, next);
      });
    }, done
  );
}
