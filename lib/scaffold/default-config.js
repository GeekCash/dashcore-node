...
/**
 * Will return the path and default geekcash-node configuration. It will search for the
 * configuration file in the "~/.geekcashcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.geekcashcore');
  var defaultConfigFile = path.resolve(defaultPath, './geekcash-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['geekcashd', 'web'];
