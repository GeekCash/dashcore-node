 ...
 * @param {String} options.dirname - The geekcashcore-node configuration directory
...
function add(options, done) {
  ...
  var geekcashcoreConfigPath = path.resolve(configPath, 'geekcashcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(geekcashcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a geekcashcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      addService(configPath, service, function(err) {
        ...
        // add service to geekcashcore-node.json
        addConfig(geekcashcoreConfigPath, serviceName, next);
      });
    }, done
  );
}
